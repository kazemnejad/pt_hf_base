...
def get_cuda_info():
    ...
    if result != CUDA_SUCCESS:
        ...
        logger.warning("cuInit failed with error code %d: %s" % (result, error_str.value.decode()))
        return []
    result = cuda.cuDeviceGetCount(ctypes.byref(nGpus))
    if result != CUDA_SUCCESS:
        cuda.cuGetErrorString(result, ctypes.byref(error_str))
        logger.warning("cuDeviceGetCount failed with error code %d: %s" % (result, error_str.value.decode()))
        return []
    for i in range(nGpus.value):
        result = cuda.cuDeviceGet(ctypes.byref(device), i)
        if result != CUDA_SUCCESS:
            cuda.cuGetErrorString(result, ctypes.byref(error_str))
            logger.warning("cuDeviceGet failed with error code %d: %s" % (result, error_str.value.decode()))
            return []
        print("Device: %d" % i)
        obj = {}
        ...
        if cuda.cuDeviceGetAttribute(ctypes.byref(cores), CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT,
            ...
            obj['cuda_core'] = (cores.value * ConvertSMVer2Cores(cc_major.value, cc_minor.value) or "unknown")
        ...
        if result != CUDA_SUCCESS:
            ...
            if result == CUDA_SUCCESS:
                ...
                cuda.cuGetErrorString(result, ctypes.byref(error_str))
                logger.warning("cuMemGetInfo failed with error code %d: %s" % (result, error_str.value.decode()))
